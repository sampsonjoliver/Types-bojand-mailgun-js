...
describe("mailgun-js", function() {
  ...
  describe("Messages", function() {
    ...
    it("test messages().send() with invalid attachment should go ok", function(
      done
    ) {
      var msg = clone(fixture.message);
      msg.attachment = 123;
    ...
    it("test messages().send() with attachment using filename with important flag", function(
      done
    ) {
      var msg = clone(fixture.message);
      msg.important = true;
    ...
    it("test messages().send() with attachment using filename and undefined v: var", function(
      done
    ) {
      var msg = clone(fixture.message);
      var filename = path.join(__dirname, "/mailgun_logo.png");
    ...
    it("test messages().send() with attachment using file buffer", function(
      done
    ) {
      var msg = clone(fixture.message);
    ...
    it("test messages().send() with attachment using Attachment object (Buffer)", function(
      done
    ) {
      var msg = clone(fixture.message);
    ...
    it("test messages().send() with attachment using Attachment object (file)", function(
      done
    ) {
      var msg = clone(fixture.message);
    ...
    it("test messages().send() with invalid inline attachment should go ok", function(
      done
    ) {
      var msg = clone(fixture.message);
      msg.inline = 123;
    ...
    it("test messages().send() with inline attachment using filename", function(
      done
    ) {
      var msg = clone(fixture.message);
      var filename = path.join(__dirname, "/mailgun_logo.png");
    ...
    it("test messages().send() with inline attachment using file buffer", function(
      done
    ) {
      var msg = clone(fixture.message);
    ...
    it("test messages().send() with inline attachment using Attachment object (Buffer)", function(
      done
    ) {
      var msg = clone(fixture.message);
    ...
    it("test messages().send() with inline attachment using Attachment object (file)", function(
      done
    ) {
      var msg = clone(fixture.message);
    ...
    it("test messages().send() with multiple inline attachments", function(
      done
    ) {
      var msg = clone(fixture.message);
      var filename = path.join(__dirname, "/fixture.json");
    ...
    it("test messages().send() with multiple inline and normal attachments", function(
      done
    ) {
      var msg = clone(fixture.message);
      var filename = path.join(__dirname, "/fixture.json");
    ...
    it("test messages().send() with multiple tags and normal attachment", function(
      done
    ) {
      var msg = clone(fixture.message);
      var filename = path.join(__dirname, "/fixture.json");
    ...
    it("test messages().send() with attachment using Attachment object (stream)", function(
      done
    ) {
      var msg = clone(fixture.message);
  ...
  describe("Domains", function() {
    ...
    it("test domains().create() invalid missing smtp password", function(done) {
      mailgun
        .domains()
        .create({ name: fixture.new_domain.name }, function(err, body) {
          assert.ok(err);
    ...
    it("test domains().credentials().list()", function(done) {
      mailgun
        .domains(fixture.existing_domain.name)
        .credentials()
    ...
    it("test domains().credentials().create() missing password", function(
      done
    ) {
      mailgun
        .domains(fixture.existing_domain.name)
        .credentials()
        .create({ login: fixture.credentials.login }, function(err, body) {
          assert.ok(err);
    ...
    it("test domains().credentials().create() invalid login type", function(
      done
    ) {
      mailgun
        .domains(fixture.existing_domain.name)
    ...
    it("test domains().credentials().create() invalid password type", function(
      done
    ) {
      mailgun
        .domains(fixture.existing_domain.name)
    ...
    it("test domains().credentials().update() missing password", function(
      done
    ) {
      mailgun
        .domains(fixture.existing_domain.name)
        .credentials(fixture.credentials.login)
    ...
    it("test domains().credentials().update() invalid password type", function(
      done
    ) {
      mailgun
        .domains(fixture.existing_domain.name)
        .credentials(fixture.credentials.login)
        .update({ password: 123 }, function(err, body) {
          assert.ok(err);
          assert(/Invalid parameter type./.test(err.message));
          done();
        });
    ...
    it("test domains().credentials().update()", function(done) {
      mailgun
        .domains(fixture.existing_domain.name)
        .credentials(fixture.credentials.login)
        .update({ password: fixture.credentials.password }, function(
          err,
          body
        ) {
          assert.ifError(err);
  ...
  describe("Unsubscribes", function() {
    ...
    it("test unsubscribes().create() missing tag", function(done) {
      mailgun
        .unsubscribes()
        .create({ address: fixture.unsubscribe.address }, function(err, body) {
          assert.ok(err);
    ...
    it("test unsubscribes().create() invalid address type", function(done) {
      mailgun
        .unsubscribes()
        .create({ address: 123, tag: fixture.unsubscribe.tag }, function(
          err,
          body
        ) {
          assert.ok(err);
    ...
    it("test unsubscribes().create() invalid tag type", function(done) {
      mailgun
        .unsubscribes()
        .create({ address: fixture.unsubscribe.address, tag: 10 }, function(
          err,
          body
        ) {
          assert.ok(err);
    ...
    it("test unsubscribes().info()", function(done) {
      mailgun
        .unsubscribes(fixture.unsubscribe.address)
  ...
  describe("Mailing lists", function() {
    ...
    it("test lists().update()", function(done) {
      ...
      mailgun
        .lists(fixture.mailingList.address)
        .update({ name: name, description: desc }, function(err, body) {
          assert.ifError(err);
          assert.ok(body.message);
          assert.equal(fixture.mailingList.address, body.list.address);
    ...
    it("test lists().members().create() invalid address type", function(done) {
      mailgun
        .lists(fixture.mailingList.address)
        .members()
        .create({ address: 1234 }, function(err, body) {
          assert.ok(err);
          assert(/Invalid parameter type./.test(err.message));
          done();
        });
    ...
    it("test lists().members().pages().page()", function(done) {
      mailgun
        ...
        .members()
        .pages()
        .page({ page: "first" }, function(err, body) {
          assert.ifError(err);
    ...
    it("test lists().members().add() without upsert", function(done) {
      ...
      mailgun
        .lists(fixture.mailingList.address)
        .members()
        .add({ members: members }, function(err, body) {
          assert.ifError(err);
          assert.ok(body.list);
  ...
  describe("Campaigns", function() {
    ...
    it("test campaigns().update()", function(done) {
      mailgun
        .campaigns(fixture.campaign.id)
        .update({ name: fixture.campaign.newName }, function(err, body) {
          assert.ifError(err);
          assert.ok(body);
          assert.ok(body.message);
          assert.ok(body.campaign);
  ...
  describe("Stats", function() {
    ...
    it("test mailgun.stats().list() with arguments", function(done) {
      mailgun
        .stats()
        .list({ event: ["sent", "delivered"] }, function(err, body) {
          assert.ifError(err);
          assert.ok(body.total_count);
          assert.ok(body.items);
          done();
  ...
  describe("Generic requests functions", function() {
    ...
    it("test mailgun.get() using promises 2", function(done) {
      ...
      mg
        .get("/address/validate", { address: "emailtestmg@gmail.com" })
        .then(body => {
          assert.ok(body);
          done();
        })
  ...
  describe("Retry", function() {
    if (
      ("messages().send() should work with retry option",
      function(done) {
        var mg = new mailgun.Mailgun({
          apiKey: auth.api_key,
          domain: auth.domain,
    ...
    if (
      ("messages().send() should retry when request fails",
      function(done) {
  ...
  describe("Complaints", function() {
    ...
    it("test complaints().delete()", function(done) {
      mailgun
        .complaints(fixture.complaints.address)
        .delete(function(err, body) {
          assert.ifError(err);
          assert.ok(body.message);
          done();
  ...
  describe("Email Validation", function() {
    it("test mailgun.validate() should validate one email address", function(
      done
    ) {
      var mg = new mailgun.Mailgun({
        apiKey: auth.api_key,
    ...
    it("test mailgun.validate() should validate one email address using private option", function(
      done
    ) {
      var mg = new mailgun.Mailgun({
        apiKey: auth.api_key,
    ...
    it("test mailgun.parse() should parse list of email addresses", function(
      done
    ) {
      var mg = new mailgun.Mailgun({
        apiKey: auth.api_key,
    ...
    it("test mailgun.parse() should parse list of email addresses with an option", function(
      done
    ) {
      var mg = new mailgun.Mailgun({
        apiKey: auth.api_key,
      ...
      mg.parse(["raboof@gmail.com"], { syntax_only: false }, function(
        err,
        body
      ) {
    ...
    it("test mailgun.parse() should private parse list of email addresses with an option", function(
      done
    ) {
      var mg = new mailgun.Mailgun({
        apiKey: auth.api_key,
      ...
      mg.parse(["raboof@gmail.com"], true, { syntax_only: false }, function(
        err,
        body
      ) {
        assert.ifError(err);
    ...
    it("test mailgun.parse() should parse list of email addresses using private option", function(
      done
    ) {
      var mg = new mailgun.Mailgun({
        apiKey: auth.api_key,
